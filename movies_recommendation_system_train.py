...
def get_recommendations(title, n_recommendations=10):
    ...
    # Mengambil 10 film yang paling mirip (selain film itu sendiri)
    movie_indices = indices_nn.flatten()[1:]

    return df['title'].iloc[movie_indices]

# Fungsi untuk menghitung precision
def precision_at_k(recommended_titles, relevant_titles):
    # Menghitung berapa banyak film yang direkomendasikan yang juga ada di list film relevan
    relevant_recommended = [title for title in recommended_titles if title in relevant_titles]

    # Precision adalah jumlah film relevan yang direkomendasikan dibagi dengan total rekomendasi
    precision = len(relevant_recommended) / len(recommended_titles)

    return precision

# Contoh penggunaan
# Misalnya, kita memiliki film 'Batman' dan daftar film yang dianggap relevan untuk evaluasi
relevant_titles = [
...
# Contoh prediksi rating untuk userId=1 dan movieId=542
svd.predict(1, 542)

# Fungsi untuk mendapatkan top-n rekomendasi berdasarkan userId
def get_top_n_recommendations(userId, n=10):

    # Mendapatkan semua movieId dalam dataset
    all_movie_ids = ratings['movieId'].unique()

    # Mendapatkan movieId yang sudah ditonton oleh user
    watched_movie_ids = ratings[ratings['userId'] == userId]['movieId'].unique()

    # Mendapatkan daftar movieId yang belum ditonton
    ...
    predictions = [svd.predict(userId, movie_id) for movie_id in unseen_movie_ids]

    # Mengurutkan prediksi berdasarkan rating tertinggi
    top_n_predictions = sorted(predictions, key=lambda x: x.est, reverse=True)[:n]

    # Mengambil top-n movieId dan estimasi rating
    top_n_movie_ids = [pred.iid for pred in top_n_predictions]
    top_n_ratings = [pred.est for pred in top_n_predictions]

  # Mengambil judul film berdasarkan id, dan mengubah title yang hilang menjadi None
    titles = df[df['id'].isin(top_n_movie_ids)]['title'].tolist()
...
# Contoh mendapatkan 10 rekomendasi teratas untuk userId=1
top_10_recommendations = get_top_n_recommendations(userId=1, n=10)

# Menampilkan hasil rekomendasi
print(top_10_recommendations)

"""Melalui evaluasi model menggunakan **cross-validation**, kita mendapatkan pemahaman yang lebih mendalam mengenai performa model dalam memprediksi rating film. Penggunaan metrik seperti **RMSE** (Root Mean Squared Error) dan **MAE** (Mean Absolute Error) memberikan gambaran tentang seberapa jauh prediksi model dari rating yang sebenarnya. RMSE yang lebih rendah menunjukkan bahwa model dapat memprediksi rating dengan lebih akurat, sementara MAE menunjukkan rata-rata kesalahan prediksi yang dihasilkan oleh model. Kedua metrik ini mengukur efektivitas pendekatan **Collaborative Filtering**, khususnya dalam memanfaatkan interaksi antar pengguna untuk memberikan rekomendasi yang relevan dan akurat.

Dengan melatih model pada seluruh dataset, kita memaksimalkan informasi yang tersedia dari pola rating pengguna. Hal ini memungkinkan model untuk menangkap lebih banyak pola tersembunyi dalam preferensi pengguna, yang pada gilirannya meningkatkan akurasi prediksi rating. Misalnya, prediksi rating untuk kombinasi tertentu dari **userId** dan **movieId** memberikan wawasan tentang bagaimana seorang pengguna mungkin bereaksi terhadap film yang belum mereka tonton. Ini merupakan salah satu keunggulan **Collaborative Filtering**, yang mampu memanfaatkan kesamaan antar pengguna untuk memberikan rekomendasi yang lebih personal dan spesifik.

Selain itu, dengan menampilkan **top-n rekomendasi** berdasarkan pendekatan **Collaborative Filtering** menggunakan model **SVD** (Singular Value Decomposition), kita dapat memberikan daftar film yang paling sesuai untuk seorang pengguna. Fungsi ini tidak hanya memprediksi rating untuk film yang belum ditonton, tetapi juga memberikan **daftar film dengan prediksi rating tertinggi**. Dengan begitu, pengguna akan menerima rekomendasi film yang paling relevan dan berkualitas tinggi, meningkatkan kepuasan mereka terhadap sistem rekomendasi.

Dengan demikian, evaluasi ini tidak hanya menunjukkan bahwa model **Collaborative Filtering** dapat memprediksi rating dengan baik, tetapi juga bahwa model ini mampu memberikan rekomendasi yang lebih personal dan disesuaikan dengan preferensi pengguna. Menampilkan **top-n rekomendasi** menjadi langkah kunci dalam memberikan nilai tambah bagi pengguna, memungkinkan mereka untuk menemukan film baru yang sesuai dengan selera mereka berdasarkan pola preferensi pengguna lain yang serupa.
